...
const ViewMenu = () => {
  ...
  // Lọc theo loại
  const starters = menus.filter(item => item.Loai === "starters");
  const mainMeals = menus.filter(item => item.Loai === "main-meals");
  const desserts = menus.filter(item => item.Loai === "desserts");
